...
- **대상 버전**: openproject-abyz-taxonomy:17.5.0-0.2.31
...
| 항목 | 값 |
|---|---|
| 개발 image | `openproject-abyz-taxonomy:17.5.0-0.2.31` |
...
| 2 | Custom image 빌드 성공 | `docker image inspect openproject-abyz-taxonomy:17.5.0-0.2.31` |
...
| 11 | Image 아카이브 생성 완료 | `/backup/op-taxonomy-17.5.0-0.2.31-<date>.tar.gz` 존재 |
...
| Custom 배포 후 | `openproject-abyz-taxonomy:17.5.0-0.2.31` | 커스텀 이미지, taxonomy 활성 |
| Rollback Path A | `openproject/openproject:17.5.0` | 표준 이미지 복귀 |
| OP 업그레이드 후 | `openproject-abyz-taxonomy:<op-version>-0.2.31` | 새 버전 커스텀 이미지 |
...
docker save openproject-abyz-taxonomy:17.5.0-0.2.31 | \
  gzip > ~/workspace/backups/op-taxonomy-17.5.0-0.2.31-${BACKUP_DATE}.tar.gz
echo "이미지 아카이브 완료: op-taxonomy-17.5.0-0.2.31-${BACKUP_DATE}.tar.gz"
...
# Step 5 — .env 전환
echo "OP_IMAGE=openproject-abyz-taxonomy:17.5.0-0.2.31" > .env
cat .env   # 확인
...
OP_VERSION=17.6.0 ABYZ_VERSION=0.2.31 ./build.sh
...
echo "OP_IMAGE=openproject-abyz-taxonomy:17.6.0-0.2.31" > ~/workspace/openproject-stack/.env
...
docker inspect openproject-stack-openproject-1 --format '{{.Config.Image}}'
# 기대값: openproject-abyz-taxonomy:17.5.0-0.2.31 (또는 현재 버전)
...
```bash
cd ~/workspace/work-github/abyz-lab-pm/custom-openproject
OP_VERSION=17.5.0 ABYZ_VERSION=0.2.31 ./build.sh
```
...
docker save openproject-abyz-taxonomy:17.5.0-0.2.31 | \
  gzip > ~/workspace/backups/op-taxonomy-17.5.0-0.2.31-$(date +%Y%m%d).tar.gz

# 아카이브에서 복원 (필요 시)
docker load < ~/workspace/backups/op-taxonomy-17.5.0-0.2.31-YYYYMMDD.tar.gz
